...
\begin{ldesc}
	...
	\li[tail (rest) of \C{xs}]           tail xs
	\li[elements of \C{xs} except last]  init xs
	\li[first \C{n} elements of \C{xs}]  take n xs
	\li[excludes first \C{n} elements of \C{xs}] drop n xs
	\li[checks for \C{x} in \C{xs}]      x `elem` xs
	\li[is \C{xs} null/empty?]           null xs
	\li[size / length of \C{xs}]         length xs
	\li[invert / reverse of \C{xs}]      reverse xs
	\li[sorts \C{xs}]                    sort xs
%	\li                                  \s\s:: Ord a => [a]
	\li[pairs (x,y) from xs and ys]      zip xs ys
%										 \s\s:: [a] -> [b] -> [(a, b)]
	\li[infinite repetition of \C{xs}]   cycle xs
...
%	\li[\s\s returning new list]    \s\s:: (a -> b) -> [a] -> [b]
	\li[fold - (z `f` left) `to` right]   foldl f z xs \li
										\s\s\small{}:: (a -> b -> a) -> a -> [b] -> a
	\li[fold - right `to` (left `f` z)]   foldr f z xs \li
...
\vspace{20mm} % Pragma, move list comprehensions to next column
