...
\begin{tabular}{C@{\s$\equiv$\s}C@{\s\s\s\s$\equiv$\s}C}
...
%	\li                         (1,'a')          & ::~(Int,Char)
	\li                         ([1,2],'a')      & ::~([Int],Char)
	\li[function ($\lambda$)]   \la\X~->~\I{e}   & ::~a -> a
...
\begin{tabular}{@{}C@{\s}CR@{\s$\equiv$\s}C}
...
%	swap      & ::~(a, b) -> (b, a) & snd (\I{x},\I{y})   & (\I{y},\I{x}) \\ % Data.Tuple
	curry     & \multicolumn{3}{@{}C}{::~((a, b) -> c) -> a -> b -> c} \\
	          & \multicolumn{2}{R@{\s$\equiv$\s}}{curry (\la(\I{x},\I{y}) -> \I{e})} & \la\I{x} \I{y} -> \I{e}\\
	uncurry   & \multicolumn{3}{@{}C}{::~((a, b) -> c) -> a -> b -> c} \\
	          & \multicolumn{2}{R@{\s$\equiv$\s}}{uncurry (\la \I{x} \I{y} -> \I{e})} & \la(\I{x},\I{y}) -> \I{e} \\
...
\begin{tabular}{@{}C@{\s}C@{\hspace{3ex}}R@{\s$\equiv$\s}C}
	...
	\multicolumn{2}{@{}C}{truncate, round, ceiling, floor} &
	\multicolumn{2}{@{}C}{::~(RealFrac a, Integral b) => a -> b} \\
\end{tabular}
...
\begin{tabular}{@{}C@{\s}C@{\hspace{3ex}}R@{\s$\equiv$\s}C}
	...
	\longcall{lines "ab\la{}nc\la{}nd"} & ["ab","c","d"] \\[1ex]
	unlines & \longtype{::~[String] -> String} \\
	\longcall{~~~~~~~~~~unlines ["ab","c","d"]} & "ab\la{}nc\la{}nd" \\[1ex]
	words & \longtype{::~String -> [String]} \\
	\longcall{words "ab c d"} & ["ab","c","d"] \\[1ex]
...
\begin{tabular}{@{}C@{}C@{\hspace{3ex}}R@{\s$\equiv$\s}C}
	...
	foldl   & \multicolumn{3}{C}{::~(a -> b -> a) -> a -> [b] -> a {\small~~-- (left) `to` right}} \\
	        & & foldl \F~\X~[\Y,\Z]    & (\X~`\F`~\Y)~`\F`~\Z       \\[1ex]
	foldr   & \multicolumn{3}{C}{::~(a -> b -> b) -> b -> [a] -> b {\small~~-- right to left}} \\
...
\begin{ldesc}
	...
	\li[ignore one of the ``components'']  MyData a \verb+_+ c
\end{ldesc}
...
\begin{tabular}{CC}
\thead{if expression}  & \thead{guarded equations} \\
\I{foo} = if~\I{boolExpr}    & \I{foo} | \I{boolExpr}~ = \I{exprA} \\
\T{foo}~~~~~then~\I{exprA}   & \T{foo} | otherwise    = \I{exprB} \\
\T{foo}~~~~~else~\I{exprB}   &                                       \\[1ex]

\thead{nested if expression}      & \thead{guarded equations} \\
\I{foo} = if~\I{boolExpr1}        & \I{foo} | \I{boolExpr1} = \I{exprA} \\
\T{foo}~~~~~then~\I{exprA}        & \T{foo} | \I{boolExpr2} = \I{exprB} \\
\T{foo}~~~~~else~if~\I{boolExpr2} & \T{foo} | otherwise     = \I{exprC} \\
\T{foo}~~~~~~~~~~~~then~\I{exprB} & \\
\T{foo}~~~~~~~~~~~~else~\I{exprC} & \\[1ex]

\thead{case expression}           & \thead{function pattern matching} \\
\I{foo x} = case~\I{x}~of         & \I{foo pat1} = exprA \\
\T{foo x}~~~~~\I{pat1} -> exprA   & \I{foo pat2} = exprB \\
\T{foo x}~~~~~\I{pat2} -> exprB   & \I{foo \_~~~} = exprC \\
\T{foo x}~~~~~\_\T{at2} -> exprC  & \\[1ex]

\thead{2-variable case expression}  & \thead{function pattern matching} \\
\I{foo x y} = case~(\I{x},\I{y})~of & \I{foo pat1 patA} = exprA \\
~~(\I{pat1},\I{patA}) -> exprA      & \I{foo pat2 patB} = exprB \\
~~(\I{pat2},\I{patB}) -> exprB      & \I{foo \_~~~ \_~~~}~ = exprC \\
~~\_~~~~~~~~~~        -> exprC      & \\[1ex]

\thead{let expression}  & \thead{where clause} \\
let~\I{nameA}=\I{exprA} & \I{foo ...}~=~mainExpression \\
~~~~\I{nameB}=\I{exprB} & ~~where \I{nameA}=\I{exprA} \\
~~~~...                 & ~~~~~~~~\I{nameB}=\I{exprB} \\
in mainExpression       & ~~~~~~~~...\\[1ex]

\thead{do notation}     & \thead{desugarized do notation} \\
do \I{statement}        & \I{statement} >>\\
~~~\I{pat} <- \I{exp}   & \I{exp} >>= \la \I{pat} ->\\
...
\begin{ldesc}
	...
	\li[Bind stdin/stdout to \C{foo} (\C{\footnotesize ::~String -> String})]
	                                                     interact foo
	\li[Write \C{cs} to a file named \C{fn}]             writeFile fn cs
	\li[Append \C{cs} to a file named \C{fn}]            appendFile fn cs
...
%	\li[Write \C{cs} to channel/file \C{h} w/ a newline] hPutStrLn h cs
\end{ldesc}
