...
\cright{
	Copyright 2018--2026, Rudy Matela --
	Compiled on \today{} \\
...
% TODO: add a couple of build tools?
% TODO: add something about f strings
...
\subsection{String formatting (f-string)}

\begin{ldesc}
	\li[format \I{x} to string] f"\{x\}"
	\li[format \I{x} to string message] f"We have \{x\} items."
	\li[format \I{x} and \I{y} to string] f"x is \{x\}, y is \{y\}"
	\li[format \I{n} to hexadecimal] f"\{n\} in hexadecimal is \{n:x\}"
	\li[format \I{n} to octal] f"\{n\} in hexadecimal is \{n:o\}"
	\li[format \I{n} with six decimal places] f"\{n:06d\}"
	\li[yield \C{"pi is 3.14"}]    f"pi is \{0:2f\}"
	\li[yield \C{"pi is 003.141"}] f"pi is \{0:3.3f\}"
\end{ldesc}
